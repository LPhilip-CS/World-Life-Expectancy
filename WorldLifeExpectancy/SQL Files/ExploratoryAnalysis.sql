...
-- Is there a corelation between life expectancy and Obesity rates?
SELECT Country, ROUND(AVG(LifeExpectancy), 1) AS LifeExp, ROUND(AVG(BMI), 1) AS ObesityRates
FROM WorldLifeExpectancy
GROUP BY Country
HAVING LifeExp > 0 AND ObesityRates > 0
ORDER BY ObesityRates DESC;


-- What is the most recent average life expectancy of the whole world?
-- @block
SELECT ROUND(AVG(LifeExpectancy), 1) AS WorldAvgLifeExp2022
FROM (SELECT Country, Year, LifeExpectancy
FROM WorldLifeExpectancy
WHERE Year = '2022') AS LifeExp2022;
